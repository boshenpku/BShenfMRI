...
for subj=1:length(subjlist)
    ...
    for jj=1:length(ROIs)
        samp=1;% number of intrapolation
        for run = 1:3
            for agent = {'sw' 'ow'}
                for context = {'bear1' 'bear2' 'bear3'}
    ...
    save(fullfile(tCoursedir,'tCourse'),'bear1sw','bear2sw','bear3sw','bear1ow','bear2ow','bear3ow');
    % combine subject
    for agent = {'sw' 'ow'}
        for context = {'bear1' 'bear2' 'bear3'}
...
for i=1:length(ROIs)
    ...
    legend('bear1sw','bear2sw','bear3sw','bear1ow','bear2ow','bear3ow','Location','Best');
